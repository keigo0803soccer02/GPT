...
var powerFlg : boolean; // スーパースピードモードのパワーが残っているかどうか

class CharacterMotorMovement {
	// The maximum horizontal speed when moving
	var maxForwardSpeed : float = 10.0;
	var maxForwardSuperSpeed : float = 15.0;
	var maxSidewaysSpeed : float = 10.0;
	var maxBackwardsSpeed : float = 10.0;
	
	var superSpeed : boolean = false; //　カスタマイズ
...
function SetPowerFlg(flg : boolean){
	powerFlg = flg;
}
...
var length : float;
    ...
    if (Input.GetKey(KeyCode.RightShift) || Input.GetKey(KeyCode.LeftShift)) { // カスタマイズ
    	if(powerFlg) {
	    	superSpeed = true;
			SendMessage("SetSpeedMode", true);
		}
    }
    else{
    	superSpeed = false; 
 		SendMessage("SetSpeedMode", false);   
    }
    
	if (desiredMovementDirection == Vector3.zero)
		return 0;
		
	else  if (superSpeed) { // カスタマイズ
		zAxisEllipseMultiplier = (desiredMovementDirection.z > 0 ? movement.maxForwardSuperSpeed : movement.maxBackwardsSpeed) / movement.maxSidewaysSpeed;
